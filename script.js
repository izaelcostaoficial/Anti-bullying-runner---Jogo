...
function defaultState() {
  return {
    ...
    settings: { lang: 'pt', theme: 'arcade', mode: 'dark', musicVolume: 70, sfxVolume: 80, muted: false }
  };
}
...
function applyI18nAll() {
  document.documentElement.lang = state.settings.lang === 'pt' ? 'pt-BR' : state.settings.lang;
  ...
  const textMap = {
    ...
    '.loading-footer span': 'loading.credits',
    '.menu-subtitle': 'menu.subtitle',
    '.menu-footer span': 'menu.footer',
    '#btn-play span:last-child': 'menu.play',
    '#btn-shop span:last-child': 'menu.shop',
    '#btn-ranking span:last-child': 'menu.ranking',
    '#btn-settings span:last-child': 'menu.settings',
    '#btn-howto span:last-child': 'menu.howto',
    '#screen-howto h2': 'howto.title',
    '#screen-howto .howto-card:nth-child(1) h3': 'howto.movement.title',
    '#screen-howto .howto-card:nth-child(1) p': 'howto.movement.desc',
    '#screen-howto .howto-card:nth-child(2) h3': 'howto.mobile.title',
    '#screen-howto .howto-card:nth-child(2) p': 'howto.mobile.desc',
    '#screen-howto .howto-card:nth-child(3) h3': 'howto.dilemmas.title',
    '#screen-howto .howto-card:nth-child(3) p': 'howto.dilemmas.desc',
    '#screen-shop h2': 'shop.title',
    '#screen-settings h2': 'settings.title',
    '#screen-ranking h2': 'ranking.title',
    '#modal-login h2': 'login.title',
    '#modal-login p': 'login.desc',
    '#btn-login-confirm': 'login.confirm',
    '#pause-title': 'pause.title',
    '#btn-resume span:last-child': 'pause.resume',
    '#btn-restart span:last-child': 'pause.restart',
    '#btn-pause-settings span:last-child': 'pause.settings',
    '#btn-pause-exit span:last-child': 'pause.exit',
    '#gameover-title': 'gameover.title',
    '#btn-gameover-retry span:last-child': 'gameover.retry',
    '#btn-gameover-menu span:last-child': 'gameover.menu',
    '#quiz-title': 'dilemma.tag'
  };
  Object.keys(textMap).forEach(function (selector) {
    const el = document.querySelector(selector);
    if (el) el.textContent = t(textMap[selector]);
  });

  const tabKeys = { skins: 'shop.tab.skins', powerups: 'shop.tab.powerups', themes: 'shop.tab.themes' };
  document.querySelectorAll('.shop-tab').forEach(function (tab) {
    const label = tab.querySelector('span:last-child');
    const key = tab.getAttribute('data-tab');
    if (label && tabKeys[key]) label.textContent = t(tabKeys[key]);
  });

  const themeKeys = { arcade: 'settings.theme.arcade', forest: 'settings.theme.forest', ocean: 'settings.theme.ocean', sunset: 'settings.theme.sunset' };
  document.querySelectorAll('#theme-options .settings-option').forEach(function (btn) {
    const key = themeKeys[btn.getAttribute('data-theme')];
    if (key) btn.textContent = t(key);
  });

  $('input-login-name') && ($('input-login-name').placeholder = t('login.placeholder'));
  $('input-player-name') && ($('input-player-name').placeholder = t('login.placeholder'));
...
function bindNavigation() {
  ...
  $('btn-shop').addEventListener('click', function () { sfxClick(); renderAllShopGrids(); showScreen('screen-shop'); });
  ...
  document.querySelectorAll('.btn-back').forEach(function (btn) {
    btn.addEventListener('click', function () { sfxClick(); showScreen('screen-menu'); });
  });
  ...
  $('btn-theme-mode').addEventListener('click', function () {
    sfxClick();
    applyColorMode(state.settings.mode === 'light' ? 'dark' : 'light');
    saveState();
  });
...
function runLoadingSequence(onDone) {
  ...
  if (!fill || !percentLabel || !statusLabel || !tipLabel || !progressBar) {
    onDone();
    return;
  }
  ...
  tipLabel.textContent = t('loading.tip');
  ...
  function step(now) {
    ...
    lastTime = now;
    progress = Math.min(100, progress + Math.max(2.5, delta * 0.085));
...
function applyColorMode(mode) {
  ...
  const toggle = $('btn-theme-mode');
  if (toggle) toggle.setAttribute('data-mode', nextMode);
  refreshThemeColorCache();
}
...
const PICKUP_WEIGHTED = ['coin', 'coin', 'coin', 'coin', 'gem', 'gem', 'power'];
...
function computeRampSpeed(elapsedMs) {
  const seconds = elapsedMs / 1000;
  if (seconds <= 30) return lerp(CONFIG.BASE_SPEED, 5.0, seconds / 30);
  if (seconds <= 60) return lerp(5.0, 6.8, (seconds - 30) / 30);
  if (seconds <= 120) return lerp(6.8, 9.8, (seconds - 60) / 60);
  return lerp(9.8, CONFIG.MAX_SPEED, clamp((seconds - 120) / 120, 0, 1));
}
...
function updateGame(dt) {
  ...
  rs.gameSpeed = computeRampSpeed(rs.elapsedMs);
  const effectiveSpeed = clamp(rs.gameSpeed * speedMult, 2.2, CONFIG.MAX_SPEED * 1.25);
  rs.distanceScroll += effectiveSpeed;

  const xpMultiplier = rs.activePowers.speed ? 2 : 1;
  rs.score += effectiveSpeed * 0.028 * xpMultiplier;
...
function drawPlayer() {
  ...
  const legAngles = [[25, -25], [-10, 10], [-25, 25], [-10, 10]];
  ...
  ctx.beginPath();
  ctx.moveTo(-12, -35);
  ctx.lineTo(-24, -18 + Math.sin(frame) * 4);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(12, -35);
  ctx.lineTo(24, -18 + Math.cos(frame) * 4);
  ctx.stroke();
...
function bindControls() {
  ...
  swipeLayer.addEventListener('touchmove', function (e) { e.preventDefault(); }, { passive: false });
  swipeLayer.addEventListener('touchend', function (e) {
    ...
    else if (Math.abs(dy) > 30) dy < 0 ? doJump() : doSlide();
  }, { passive: true });
...
function hideAllModals() {
  ['modal-login', 'modal-dilemma', 'modal-pause', 'modal-gameover', 'modal-quiz'].forEach(function (id) {
    const el = $(id);
...
function triggerGameOver() {
  ...
  if ($('modal-dilemma')) $('modal-dilemma').classList.add('hidden');
  if ($('modal-pause')) $('modal-pause').classList.add('hidden');
...
function boot() {
  ...
  applyColorMode(state.settings.mode || 'dark');
  $('btn-mute').setAttribute('data-state', state.settings.muted ? 'muted' : 'unmuted');
